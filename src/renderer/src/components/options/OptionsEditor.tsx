...
import { canFilter, canOutputSize } from '@shared/options'
...
/**
 * 任务参数的**整块**面板：裁剪 + 处理链 + 输出约束，按类别显示其中几块。
 *
 ...
 * ## 三块的顺序是固定的
 *
 * 裁剪 → 处理链 → 输出约束，与 `TaskOptions` 里字段的声明顺序、以及摘要那一行的
 * 拼接顺序**一致**。三处同序不是洁癖：用户在面板里从上往下读到的次序，
 * 与卡片摘要上从左往右读到的次序对不上时，他会怀疑自己记错了参数。
 *
 * ⚠️ **三块面板各自独立 apply，但 `setOptions` 是整体替换语义**，所以每一块都必须用
