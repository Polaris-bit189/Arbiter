...
/**
 * 省略质量档时用的恒定质量与速度档。**就是加这个参数之前写死在本文件里的那两个数**，
 * 所以「没设过质量档的任务产物一个字节都不变」是结构上的，不是靠断言祈祷。
 *
 * ⚠️ **这里刻意用字面量，不从 `@shared/options` import 那两个同名常量。**
 * 本文件是 `test:core` 直接加载的，而那个套件跑的是裸 `tsx scripts/test-core.ts`
 * （**不带 `--tsconfig`**，见 `package.json`），因此解析不了 `@shared/*` 这个别名——
 * 而 `@shared/options` 里其余内容全是**值**，加一行 import 会让那个套件在运行时
 * `Cannot find module '@shared/options'`。（`ffmpegRun.ts` 早就 import 了同一个模块的
 * `MIN_BITRATE_KBPS`，它没事，因为加载它的 `test:tasks` 带了 `--tsconfig`。）
 *
 * 两份数的漂移由 `scripts/test-core.ts` 的 `[10]` 里一条断言钉住：
 * 「引擎默认值 = `@shared/options` 的 `DEFAULT_CRF` / `DEFAULT_PRESET`」。
 * 谁把那个套件改成带 `--tsconfig` 跑，就可以把这两个常量删掉直接 import。
 */
