...
| **Archive**  | `zip` `7z` `tar` `rar` `iso` `tar.gz` …         | 7-Zip                           |
...
converts a `.docx` / `.xlsx` / `.odt` / `.epub` to Markdown on the fly when the
agent Reads one. One install, no config files.
...
- **Engines run as subprocesses, always.** No FFI, no in-process decoding. A
  blocked main process means no progress bar, no cancel button, and an app that
  looks hung, so an engine that can't yield is an engine that doesn't ship.
...
There are eighteen test suites, which `npm test` runs, plus a set of
...
- [x] Smart remuxing: when the codecs are already container-compatible, copy the
      streams instead of re-encoding them (33.9× faster on a 1080p sample)
- [x] GPU acceleration (NVENC), opt-in, with CPU fallback
- [x] MCP server + Claude Code plugin
- [x] Video trimming without re-encoding (lossless mode copies the stream; the
      start snaps to the nearest keyframe and the app says so)
