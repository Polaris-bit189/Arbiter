...
npm run test:plugin-target # 插件「找应用」30 条：解析 reg.exe 的卸载表输出（DisplayName 判定 /
                     #   带空格与引号的路径 / GBK 与严格 UTF-8 解码）+ 真注册表临时键往返
                     #   （**父键 + {GUID} 子键**，与生产同形）+ 候选次序 + **端到端**
                     #   （resolveTarget 真的挑中/挑不中）。它是唯一盯得住「装到了默认位置
                     #   之外」那件事的套件（约束 21 那一族）
npm run test:anchors # 锚点预检（16 个 falsify 脚本 / 203 个锚点，约 5 秒）：只验「变异还找得到目标」，
...
**⚠️ 同一族的第三个实例（2026-09-17 实测）：`candidateRoots()` 只认三个**默认**安装位置，
安装时改了目录就等于「没装」。**

- **现象**：应用装在 `D:\tools\Arbiter`（安装向导里改的），于是
  `%LOCALAPPDATA%\Programs\Arbiter` / `%ProgramFiles%\Arbiter` /
  `%ProgramFiles(x86)%\Arbiter` **一个都不存在** → `resolveTarget()` 返回 **null** →
  插件的 MCP server 报「没有在本机找到可用的 Arbiter」并退出。而应用装得好好的、
  `arbiter.cmd --version` 也正常。
- **为什么又是同一个形状**：开发机上 `candidateRoots()` 里有**仓库根**那条候选兜着，
  所以 `npm run mcp` 与插件端到端**照样全通**。这条是**用户把应用装到 D 盘**才撞出来的
  ——小 C 盘上很常见，不是假想的边角。
- **修法**：`registryRoots()` 去 `HKCU` / `HKLM` 的 `…\CurrentVersion\Uninstall` 里问一句。
  两条判据都要：**`DisplayName` 恰好是 `Arbiter`**（`/f` 是子串匹配，`Arbiter Beta`
  这类邻居条目会一起被捞出来，然后我们会去 probe 一个**别人的**目录）；**目录从
  `DisplayIcon` / `UninstallString` 反推，先剥引号再按 `.exe` 截**（路径里可能有空格，
  顺序反了会在半路断掉）。输出是 GBK，解码照约束 23 那套。代价实测（各 5 次取中位）：
  `HKCU` 那次 **17.8 ms**、`HKLM` **36.3 ms**，一次 `registryRoots()` 合计 **约 77 ms**
  （两个键各起一个 `reg.exe`）——这条链路每次都要起一个 Electron，这点钱可以忽略。
  ⚠️ 第一版这里写的是「约 31 ms」，那是我只量了一条键的口径，审计复量时对不上。
- **次序是承重的**：注册表那条排在三个标准位置**之后**、开发形态（仓库根 / cwd）**之前**。
  反过来的话，用户机器上只要恰好 clone 过一个仓库，装好的应用就被顶掉了。
- **守卫**：`scripts/test-plugin-target.mjs`（30 条，纯逻辑 + 一次临时注册表键往返，
  **进聚合**——它不碰构建产物，秒级）+ `scripts/falsify-plugin-target.mjs`
  （14 个变异，单轮实测 **9.9 秒**，也在聚合里）。
- ⚠️ **第一版的覆盖是虚的，这是审计量出来的**：测试注入的是**叶子键**，而生产注入的是
  **父键**（条目在 `…\Uninstall` 的 `{GUID}` 子键下），于是 `reg query` 的 `/s`
  （递归子键）**拿掉都不会红**——而生产形态下少了它就是查不到，**整条修复静默失效**。
  审计另外点出 7 个「改了它、一个都不会红」的支柱（`DisplayIcon`/`UninstallString`
  的优先级、`.exe` 结尾判据、`probe()` 的 `usable === false`、三个标准安装位置、
  严格 UTF-8 那一步、`ARBITER_HOME` 的短路、跨键去重），都补了断言与变异。
  **教训与约束 30 那条同源：测试的注入形状必须与生产一致，否则变异覆盖是假的。**
- ⚠️ **我原先在这里写过一段「端到端断言会假红」的理由，那是错的（同一次审计证伪）。**
  原文说：本仓库里 `REPO_ROOT` 永远是仓库自己，所以「`resolveTarget()` 选中自定义目录
  里那份应用」这条断言写了就会在开发机上假红。**实测：本机 `REPO_ROOT` 存在且
  `usable: true`，而 `resolveTarget()` 照样选中了 `D:\tools\Arbiter`** —— 注册表那条
  排在开发形态之前，它赢是必然的。真正的理由只是当时 `resolveTarget()` 内部**裸调**
  `candidateRoots()`、**没有注入点**。按「一条推理没实测过就不配写成结论」（与 21b 同源），
  这里如实更正。现在 `resolveTarget(entry, uninstallKeys)` 开了注入点，端到端**有覆盖**：
  注入临时键 → 必须挑中那份假安装；注入不匹配的键 → 必须挑不中（反向对照）。
- 顺带一条：卸载表里可能留着**指向已删除目录**的旧条目（本机就有一条，来自早先的
  临时安装）。它无害——`probe()` 对不存在的根返回 null，轮不到它。
- **已知边界，如实记下、这次不修**（同一次审计指出）：
  - `resolveTarget()` 取的是**第一个 `usable`**，所以「两份安装同时在盘上」时谁赢由候选
    次序决定：一份**旧的、残留在标准位置**的副本会压过注册表里**新装到别处**的那份
    （NSIS 重装换目录时**不会**卸载旧的）。多条注册表条目之间同理，是按**键名字符串序**
    排的（`/s` 按名字枚举），不是按新旧。两条都不是这次改动引入的（改之前只知道标准位置，
    情况一样）；真要解决得比版本或 mtime，那是一次独立决策，**别顺手做**。
  - `read-convert.mjs` 的 hook 在**缓存短路之前**就调 `resolveTarget()`，于是每次命中
    扩展名的 Read 都要起两个 `reg.exe`（合计约 60~80 ms），哪怕产物是复用的、这一轮
    压根不会再起 CLI。绝对值可接受（同一条链路本来就可能要起 Electron），但它是**每次
    Read 的固定税**。
...
**41. 编码质量档（P0-10）：三项都实测过，而**第一条实测结论就推翻了直觉**。（实测，2026-09-17）

`TaskOptions.quality = { crf?, preset?, tune? }`（HandBrake 那个 Video 页的三件套）。
**三个字段都可省，一个都不给 = 加这个参数之前的行为**（`-c:v libx264 -preset veryfast -crf 23`），
所以没设过质量档的任务产物**一个字节都不变**——这一条是结构上的（`x264Args` 的默认值就是
原先写死在 `videoArgs` 里的那两个数），并且有断言钉着。

### 三件实测出来的事

| preset    | 时间（1080p30/2s） | 体积    | SSIM   | 峰值 RSS |
| --------- | ------------------ | ------- | ------ | -------- |
| ultrafast | 347 ms             | 3068 KB | 0.9867 | 887 MiB  |
| veryfast  | 577 ms             | 1356 KB | 0.9883 | 1544 MiB |
| fast      | 902 ms             | 1631 KB | 0.9901 | 1759 MiB |
| medium    | 1022 ms            | 1625 KB | 0.9901 | 2007 MiB |
| slow      | 1278 ms            | 1498 KB | 0.9897 | 2125 MiB |
| veryslow  | 3796 ms            | 1299 KB | 0.9896 | 2525 MiB |
| placebo   | （更慢）           | —       | —      | **3556 MiB** |

（真实照片推镜素材，`-crf 23`。体积那一列的**非单调**在 640x640 那组上更明显：
`veryfast` 213 KB / `fast` 291 KB / `veryslow` 234 KB。）

- **⚠️ 「越慢的预设体积越小」是错的。** 同 CRF 下 `veryfast` 体积最小、而 **SSIM 最差**
  （0.9883，其余六档都在 0.9896~0.9901）。机制是 faster 档关掉了心理视觉优化、纯按 PSNR 编。
  **两件事由此定下来**：界面文案一个字都不能提「会更小」；「预设」与「CRF」**不能互相换算**
  ——它们是两个独立旋钮。反过来 `-crf` 自己的单调性成立（crf 18 的产物约是 crf 40 的 4~6 倍），
  那条可以断方向。
- **画质从 `fast` 往后基本不再涨**，而时间继续翻倍（`fast → veryslow` 是 4.2×，
  而 SSIM 从 0.9901 走到 0.9896——**还降了**）。`placebo` 是纯代价（比 `veryslow` 还慢，
  体积与 SSIM 都一样），留在选项里是为了「能选」，界面上写明了它的代价。
- **⚠️ 慢预设更吃内存，而这是并发那本账的新输入。** 1080p 单进程峰值 RSS 是一条干净的
  单调曲线（887 → 3556 MiB，`placebo` 是 `veryfast` 的 **2.30 倍**）。所以
  `core/queue.ts` 的 `taskCost()` 加了第三个参数（`TaskOptions`），`PRESET_MEMORY_FACTOR`
  按实测比值定档（快档 1 / medium~veryslow 1.5 / placebo 2.5），向上取整。
  大文件 × `placebo` = 8 份，容量 12 于是只开出 **1 路**。
  ⚠️ **第三个参数是必填的**（可以传 `undefined`）：做成可选的话「忘了传」不会有任何症状
  ——队列只是悄悄多放几条重编码进去。MCP 那条路今天显式传 `undefined`（它还没有质量档）。
  ⚠️ `setOptions` 改完参数**也要换掉队列条目**（在那之前「参数不参与分桶」是对的）——
  少了它的表现是那条排队中的任务在**别的任务眼里**还是轻的。

### 四个设计取舍

- **只对走 libx264 的出口开放**（`supportsQuality()`：mp4 / mkv / mov / m4v / avi）。
  webm 走 vp9，它的 CRF 是 **0~63 的另一条尺子**、速度档是 `-deadline/-cpu-used`、
  也没有 `-tune film/animation/grain`；gif 走调色板滤镜，根本没有质量旋钮。
  判据写成**排除表 + `categoryOf()`**（不是一张包含表）：包含表在能力矩阵将来加一个视频
  出口时不会自动跟上，而那种缺失没人会想到去查。`test-core.ts` 里有一条漂移守卫钉它。
- **与输出约束互斥，schema + `setOptions` + 引擎三层都拒。** `-crf` 与 `-b:v` 同时给时
  x264 会**回到质量模式**、把 `-b:v` 只当上限，产物体积与目标彻底脱钩**而任务报成功**。
  统一文案是 `QUALITY_OUTPUT_EXCLUSIVE`（UI 的禁用理由与引擎抛出的错误**是同一句**）。
  与**无损裁剪**、**`remux=force`** 同样互斥，理由同源（两条 `-c copy` 快车道都不编）。
  ⚠️ **出口判据必须在引擎层再判一次**：目标格式在参数设完之后还能改（`setTarget` 不碰
  `options`），少了它「先给 mp4 设好 CRF、再改成 webm」会静默失效、任务报成功。
- **与显卡编码互斥**：`-crf 23` 与 `-cq 23` 同号不同质（约束 22 实测差 2.8 倍体积），
  `-preset veryfast` 与 `-preset p4` 也不是同一个词汇表。所以设了质量档**不探显卡**、
  直接走 CPU，并且**说出来**（与「有输出目标时不探」落在同一个 `if` 上）。
- **`remux` 判定多了一条 `quality === undefined`。** 少了它，一条「mkv(h264) → mp4 + CRF 18」
  会走 `-c copy`——三个旋钮全被静默丢掉、产物就是源的副本，而任务报成功。

### ⚠️ 顺带挖出来的一个**潜伏了很久**的坑：`test:core` / `test:doc` 没带 `--tsconfig`

`package.json` 里二十来支测试脚本**只有这两支**是裸 `tsx scripts/xxx.ts`，其余都带
`--tsconfig tsconfig.test.json`。而根 `tsconfig.json` 只有一个 `references` 列表、
**没有 `paths`**，所以这两支解析不了 `@shared/*`——它们此前一直没炸，是因为它们加载的
模块里对 `@shared/*` 全是 **`import type`**（编译期就被抹掉）。

`engines/ffmpeg.ts` 原先正是这样。P0-10 想从 `@shared/options` 取两个默认值，一加**值导入**，
`test:core` 当场 `Cannot find module '@shared/options'`。处置：那两个默认值**就地写在引擎里**，
再由 `test-core.ts` 的 `[10]` 用一条断言把两份数钉在一起。**谁把这两支套件改成带
`--tsconfig` 跑，就可以把那份副本删掉。**

> 判据：**往 `src/main/**` 加 `@shared/*` 的值导入之前，先问一句「谁在加载它、那个套件带没带
> `--tsconfig`」**。与之配对的是：**测试脚本自己要用相对路径 import `src/shared/*`**
> （`test-core.ts` 的文件头写着）。

### 验收

`test-core.ts` 的 `[9]`（契约 / 判据 / 文案，含「能力矩阵与 `supportsQuality` 不漂移」）
与 `[10]`（参数构造，含「省略质量档 = 老行为」「两份默认值不许漂」「接线：`costOf` 传了
`task.options`」「队列条目在 `setTarget` 与 `setOptions` **两处**都换掉」）；
`test-tasks.ts` 的 `[21]`（**真编码**：CRF 的体积单调性、预设与调优真的改变码流、
质量档让 remux 失效、拒收面、与无损裁剪互斥、GPU 让路且说出来）；
`test-ui.ts` 的 `[11]`（真渲染：禁用与理由、载荷完整性与**能过契约**）。

⚠️ **两条断言的写法本身是结论**：① 「预设不同 → 产物不同」**只能**判「不同」，判方向会在真实
素材上假红；`-tune grain` 的方向可以判（结构上就该多花比特），门槛按**两份实测里小的那份**
留余量（+10%）。② 「出口不支持」那一条必须先 `setTarget` 再断言——**`setTarget` 是 `void`**，
写成 `setTarget(...) && !setOptions(...)` 左边恒为 undefined、断言**永远红**（第一版就是这么写的）。

反证：`falsify-ui-panel.mjs` 里五条（**第一次出现跨两个文件的反证**：`m.file` 缺省是处理链
面板，新增的五条落在 `QualitySection.tsx` 上）；`falsify-tasks.mjs` 里六条（质量档被忽略 /
remux 不让路 / 引擎层不判出口 / 不判无损裁剪互斥 / 照旧探显卡 / 不参与并发记账）。

⚠️ **改这一块会让两条老锚点跟丢**（锚点预检当场抓到）：`setTarget` 那句 `queue.remove/push`
因为 `setOptions` 也加了一句逐字相同的而**命中 2 次**；`taskCost` 的
`if (...) return HEAVY_COST` 换成了 `const base = ... ? ... : ...`。
**改了被变异覆盖的代码必须重跑 `npm run test:anchors`**——这两条要是漏了，只会在下一轮真跑
`falsify:tasks`（二十多分钟）时才现形。


---

**42. 加密音乐容器（P0-11）：能验到什么程度，取决于你手上有没有真样本——而**我们一个都没有**。（2026-09-17）

`.ncm`（网易云）/ `.qmc0/.qmc2/.qmc3/.qmcflac/.qmcogg/.mflac/.mflac0/.mgg/.mgg1/.mggl`（QQ 音乐）/
`.kwm`（酷我）/ `.xm`（虾米）现在都是**普通音频源**。走新引擎 `encmusic`：
先**开壳**拿回里面那个 mp3 / flac，再按需要交给 ffmpeg。**它不是解码器**——
ffmpeg 根本不认识这些容器。
...
| 情况 | 做什么 |
| --- | --- |
| 解出来的扩展名 **就是**目标格式，且任务没有任何参数 | 直接落盘，**一个字节都不经过编码器** |
| 其余（换格式，或带了裁剪 / 处理链 / 目标体积） | 解到临时文件，交给 `runFfmpeg` |

⚠️ **有参数时必须走 ffmpeg**：参数都是编码参数，而「直接落盘」那条路一个字节都不编——
静默忽略它们就是「用户填了参数、产物没变、任务报成功」。判据写在适配器里而不是界面里，
因为别的入口（MCP / 右键菜单）绕得过去。

### 三个必须记住的点

- **内含格式按字节判，不看容器里那个 `format` 字段。** 各家 App 写的元数据是它们自己的记账，
  而我们要回答的是「这堆字节是什么」。两者不一致时，按字节那个答案能让 ffmpeg 正确解码。
  认不出来时**抛错，绝不猜一个默认值**——猜错的表现是「拿去转 mp3、产物是噪音」。
- **NCM 的两处偏移是两个独立实现对上的，别动。** 音频起点是「跳过 5 字节 → 读
  `image_space` → 总跳过 `image_space + 13`」（Python 的 `ncmdump` 与 npm 的 `ncmdump` 各写一边、
  结论相同）；密钥流**要整体后移一位**（Python 写 `stream[1:]`，JS 写下标 `i+1`）。这两处各差一个
  常数，症状都是**产物全是白噪声而没有任何地方报错**。密钥块那段是 AES-128-ECB + PKCS#7，
  而 **Node 的 `createCipheriv` 默认自己会补 PKCS7**——写合成素材时手工再补一次，密文会多出
  一整块（33 → 48 → 64），两个读取器一起解不出来。这一条是**对拍时抓到的**。
- **QQ 音乐那套 TEA 全程大端**（密钥的四个字是，分组的 v0/v1 也是）。它移植自 Go 的
  `x/crypto/tea`。写成小端的话，派生出来的密钥块是一堆看着像随机的字节——
  好在 `ZERO_LEN` 那 7 个校验零会当场把它拦下。

### ⚠️ 验证只到「与独立实现一致」这一层，缺的东西要写在明处

**没有真实样本，也不会有。** 这一行的规矩是「自己能造出来的东西多半也能验自己」，
而生态里的同类项目也同样只用合成数据（`HRuiCcc/music-geshizhuanhuan` 的 README 明写
「测试样本全部为自建合成数据（正弦波），不含任何版权内容」）。所以分两层，
**两层证明的东西不一样，别混着说**：

| 层 | 做法 | 证明了什么 | 没证明什么 |
| --- | --- | --- | --- |
| `test-encmusic.ts` 的 `[1]` | 提交的 `fixtures/encmusic/sample.ncm`（合成正弦波容器）——**Python 的 `ncmdump` 解出来与 `sample.expected.wav` 逐字节相同** | 我们的读取器与**另一个实现**在版面与密钥流上一致 | 真实 App 产出的文件一定长这样 |
| `[3]` 的合成往返 | 现场造容器 → 现场解 | 读写两个方向没写岔 | 版面本身对不对（两边可能一起错） |

**只有第一层有跨实现证据。** 复现那条交叉验证要装 `pycryptodome` + `mutagen`，
**不在 CI 里**（与 `falsify:plugin-launch` 同理）——它是一次性取证，不是持续保证。
谁拿到真实样本，放进 `scripts/fixtures/encmusic/` 补一条断言即可。

### 没做的：酷狗 `.kgm` / `.kgma`

它们要一张逐字节掩码表，而我们**没能从可达的源里取到**（GitHub 直连不通；npm 上那两个包是
WASM，表在二进制数据段里，得反编译一个依赖）。处置是**不登记**，而不是登记一个必然失败的
扩展名——后者用户会以为「这个格式支持，只是我的文件坏了」。

### 一键回退（这一条是本类功能独有的）

同类项目在别处有过**下架**记录。所以「几分钟内重出一份不含它的版本」是这条路的一部分：

```
node D:\check\make-clean-copy.mjs --no-encmusic
```

规则在 `scripts/encmusic-strip.json`（**随代码走**——接线点就是代码的一部分），
动手的是 `scripts/strip-encmusic.mjs`。三条设计约束：

- **每一步都验锚点命中次数**，对不上就**整个中止**。剥不干净比剥不掉危险得多。
- **最后有一道全局复核**：在整棵树里搜 9 个特征串，**搜到一个就非零退出**。
  剥离工具自己也在那份删除名单里（否则它会砸掉自己的复核）。
- **删除放在替换之后**：替换失败时树里还留着规则，人能就地看出是哪条锚点过期了。

✅ **实测过**：拿当前工作树复制一份 → 剥离 → **`tsc --noEmit` 通过**。
第一次跑它报了两次错（锚点不唯一、`to` 里漏了冒号），都是这一步抓出来的——
「剥完能不能编译」这条检查本身值得留着。

### 验收

`test-encmusic.ts` 25 条；`falsify-encmusic.mjs` 5 个变异（都打在**往返够不着**的地方：
偏移 / 密钥流 / 「猜一个默认值」/ 分派走错 / 分支反了）。进 `test` 与 `falsify` 两个聚合。
